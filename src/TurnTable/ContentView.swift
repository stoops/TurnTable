...
extension String {
    func version() -> String { return "1.1.509" }
}
...
struct tabs: View {
                ...
                VStack(spacing:0) {
                    GeometryReader { geome in
                        ScrollViewReader { proxy in
                            ...
                            }.onChange(of:objc.isel) {
                                seek()
                            }.onChange(of:objc.isrt) {
                                seek()
                            }.onChange(of:objc.isee) {
                                seek()
                            }
                        }.foregroundColor(Color.clear).background(Color.clear)
                    }
    ...
    func cold(i:Int, j:Int, w:CGFloat, r:UUID, c:Color) -> AnyView {
        ...
        sizt = (sizt + (CGFloat(r.hashValue % 1000) / CGFloat(100000)))
            ...
            vobj = AnyView(HStack {
                if let tabo = tabz {
                    tabo.onAppear {
                        ...
                        nobj.view_tabb.isee += 1
                        refr = UUID()
                    }.onChange(of:nobj.view_sels) {
                        nobj.view_tabb.isel += 1
                        refr = UUID()
