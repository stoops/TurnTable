...
extension String {
    func version() -> String { return "1.1.109" }
}
...
struct slid: View {
    ...
    @Binding var backg: Color
    @Binding var colrb: Color

    @State var resto: Color?
    @State var dragd = Color.white.opacity(0.75)

    @State var lastcoord: CGFloat = 0.0

    var body: some View {
        GeometryReader { gr in
            let radial = (gr.size.height * 0.81)
            let radius = (gr.size.height * 0.69)
            let minval = (gr.size.width * 0.010)
            let maxval = (gr.size.width * 0.99) - radial

            ZStack {
                RoundedRectangle(cornerRadius:radius).inset(by:-1.99).stroke(backg.opacity(0.95), lineWidth:1.59).background(Color.black.opacity(0.05))
                    .gesture(
                        DragGesture(minimumDistance:0)
                            .onChanged { v in
                                if (!locks) {
                                    self.edits = true
                                    if (self.resto == nil) { self.resto = self.colrb }
                                    self.colrb = self.dragd
                            ...
                            .onEnded() { v in
                                if (!locks) {
                                    self.edits = false
                                    if (self.resto != nil) { self.colrb = self.resto! }
                                }
                            }
                        )
                HStack {
                    Circle()
                        .foregroundColor(colrb).brightness(0.05)
...
class note: NotificationCenter, @unchecked Sendable {
    ...
    func loop() {
        ...
        let sldr = \ContentView.sldr
        let edit = \ContentView.edit
        let movd = \ContentView.movd
        ...
        while (0 == 0) {
            ...
            let chks = gets()
            if (chks > 0) { objc[keyPath:sldr] = false }
            else { objc[keyPath:sldr] = true }
            ...
            if (tsec > 1) {
                if (pobj == nil) { csec = 0 }
                if (objc[keyPath:movd]) {
                    objc[keyPath:movd] = false
                    seek = 1
                }
...
struct ContentView: View {
    ...
    @State var sldb = Color.clear
    @State var sldc = Color.clear
        ...
        ZStack {
            VStack {
                HStack {
                    ...
                    HStack {
                        butv(kind:"arrow.left.circle", size:38.0, extr:[epad, 0.0], iidx:0, clst:["b", "g", "b", "!"], meth:-1, pram:-1, actn:prev)
                        butv(kind:mode, size:46.0, extr:[epad, 0.0], iidx:1, clst:["b", "g", "b", "*"], meth:-1, pram:-1, actn:bply)
                        butv(kind:"arrow.right.circle", size:38.0, extr:[epad, 0.0], iidx:2, clst:["b", "g", "b", "!"], meth:-1, pram:-1, actn:more)
                    }.frame(width:219.0).offset(x:1.99, y:0.0)
                    HStack {  }.padding(.leading, 28.0)
                    HStack {
                        ZStack {  }.zIndex(99.0).overlay(butv(kind:"shuffle.circle", size:28.0, extr:[0.0, 0.0], iidx:3, clst:["b", "g", ((shuf[0] == false) && (shuf[1] == false)) ? "b" : "g", "*"], meth:-1, pram:-1, actn:shfs).offset(x:-35.0, y:0.0))
                        ZStack {
                            RoundedRectangle(cornerRadius:19.0).stroke(colr(k:clrs[55][2]), lineWidth:3.0).fill(colr(k:"z")).frame(width:ssiz, height:88.0).overlay(
                                VStack {
                                    txtv(strs:gets(kind:0), size:17.0, colr:colr(k:"t"), kind:0, bold:0).offset(y:-7.09).frame(width:ssiz*0.89)
                                    txtv(strs:gets(kind:1), size:13.0, colr:colr(k:"t"), kind:0, bold:0).offset(y:-1.99).frame(width:ssiz*0.75)
                                    HStack {
                                        txtv(strs:time[0], size:11.0, colr:colr(k:"t"), kind:1, bold:1).padding(.trailing, 8.0).offset(y:-0.39)
                                        slid(locks:$sldr, edits:$edit, moved:$movd, value:$prog, backg:$sldb, colrb:$sldc).frame(width:ssiz*0.50, height:13.0).offset(y:-0.99).onAppear {
                                            sldb = colr(k:"t")
                                            sldc = colr(k:"b")
                                        }
                                        txtv(strs:time[1], size:11.0, colr:colr(k:"t"), kind:1, bold:1).padding(.leading, 8.0).offset(y:-0.39)
                                    }.offset(y:3.09)
                                }).offset(y:-1.09)
                    ...
                    HStack {
                        TextField("Filter", text:$srch).disabled(fals).onAppear { DispatchQueue.main.async { fals = false } }
                            ...
                            .overlay(RoundedRectangle(cornerRadius:13.0).inset(by:-5.0).stroke(colr(k:clrs[50][2]), lineWidth:3.0))
                            .offset(x:3.99)
                        butv(kind:funp(iidx:4), size:30.0, extr:[0.0, 0.0], iidx:4, clst:["b", "g", (shwp == false) ? "b" : "g", "~"], meth:-1, pram:-1, actn:clkp).offset(x:12.0).overlay(
                            butv(kind:"multiply.circle", size:24.0, extr:[epad*1.5, 3.5], iidx:5, clst:["b", "g", "b", "!"], meth:3, pram:3, actn:nill).padding(.trailing, 1.0).offset(x:13.0, y:37.00))
                    }.offset(x:0.09, y:-1.99).overlay(
                        HStack {
                            butv(kind:"star.circle", size:24.0, extr:[epad*1.5, 3.5], iidx:6, clst:["b", "g", "b", "!"], meth:-1, pram:-1, actn:star)
                            butv(kind:"viewfinder.circle", size:24.0, extr:[epad*1.5, 3.5], iidx:7, clst:["b", "g", "b", "!"], meth:-1, pram:-1, actn:fndr)
                            butv(kind:"line.3.horizontal.circle", size:24.0, extr:[epad*1.5, 3.5], iidx:8, clst:["b", "g", (tsho == "no") ? "b" : "g", "*"], meth:-1, pram:-1, actn:shot)
                            butv(kind:funs(iidx:9), size:24.0, extr:[epad*1.5, 3.5], iidx:9, clst:["b", "g", (shws == false) ? "b" : "g", "~"], meth:-1, pram:-1, actn:clks)
                        }.offset(x:-9.99, y:35.99)
                    )
                    HStack {  }.padding(.leading, 36.0)
                }.padding(EdgeInsets(top:12.0, leading:0.0, bottom:36.0, trailing:0.0))
                colv()
                HStack {
                    ZStack {
                ...
                HStack {
                    ...
                    HStack {
                        ...
                        let strv = String(format:"%@  Tracks", stts).lpadr(toLength:21, withPad:" ", padSide:0)
                        txtv(strs:strv, size:15.0, colr:colr(k:"t"), kind:3, bold:0).offset(x:0.00, y:-0.19)
                    }
                    HStack {
                        ...
                        if let imgo = imgs() {
                            Rectangle().frame(width:1.0, height:1.0).foregroundColor(noco()).padding(EdgeInsets(top:0.0, leading:19.0, bottom:0.0, trailing:19.0)).overlay(
                                imgo.resizable().frame(width:42.0, height:42.0).opacity(0.75)
                    ...
                    HStack {
                        let strv = String(format:"TurnTable  %@", String().version()).lpadr(toLength:21, withPad:" ", padSide:1)
                        txtv(strs:strv, size:15.0, colr:colr(k:"t"), kind:3, bold:0).offset(x:0.00, y:-0.19)
                    }
                    Spacer()
                    HStack {
                        ProgressView().scaleEffect(x:0.69, y:0.69, anchor:.center)
                    }.offset(x:-5.0, y:3.0).opacity(opap)
                }.padding(.bottom, 5.0).offset(x:-5.99, y:-5.99).onHover { over in
                    shwp = false
                    clrs[3][2] = clrs[3][0]
                }
            }.background(bgfu(k:1))
    ...
    func bgfu(k:Int) -> AnyShapeStyle {
        ...
        let dark = Color.init(red:0.35, green:0.35, blue:0.35)
        if (wini[0] <= 60.0) { wini[0] = 97.0 }
        if (k == 0) {
            if (iidx > 0) { return AnyShapeStyle(dark.opacity(0.55)) }
    ...
    func colr(k:String) -> Color {
        var offs = 0.0
        if (wini[0] <= 60.0) { wini[0] = 97.0 }
        if ((k.count > 1) && k.hasSuffix("h")) { offs = 0.19 }
        let acol = Color.init(red:0.17+offs, green:0.17+offs, blue:0.17+offs, opacity:wini[0]/100)
        ...
        let wcol = Color.init(red:0.13+offs, green:0.13+offs, blue:0.13+offs, opacity:0.53)
        let scol = Color.init(red:0.17+offs, green:0.17+offs, blue:0.17+offs, opacity:0.97)
        var gcol = tcol.opacity(0.75)
        var lcol = tcol.opacity(0.79)
        let zcol = tcol.opacity(0.09)
        if (cbco != Color.clear) {
            let nsco = NSColor(cbco)
            bcol = Color.init(red:nsco.redComponent+offs, green:nsco.greenComponent+offs, blue:nsco.blueComponent+offs, opacity:0.95)
        }
        if (chco != Color.clear) {
            let nsco = NSColor(chco)
            gcol = Color.init(red:nsco.redComponent+offs, green:nsco.greenComponent+offs, blue:nsco.blueComponent+offs, opacity:0.95)
        }
        if (ctco != Color.clear) {
            let nsco = NSColor(ctco)
            tcol = Color.init(red:nsco.redComponent+offs, green:nsco.greenComponent+offs, blue:nsco.blueComponent+offs, opacity:0.95)
            lcol = Color.init(red:nsco.redComponent+offs, green:nsco.greenComponent+offs, blue:nsco.blueComponent+offs, opacity:0.79)
        }
    ...
    func main() {
        ...
        isfo = true
        shuf[1] = false
        if (cbno[0] != 0.0) { cbco = Color(red:cbno[1], green:cbno[2], blue:cbno[3]) }
        if (chno[0] != 0.0) { chco = Color(red:chno[1], green:chno[2], blue:chno[3]) }
        if (ctno[0] != 0.0) { ctco = Color(red:ctno[1], green:ctno[2], blue:ctno[3]) }
    }
    ...
    func symb(r:Int) {
        ...
        mode = retl[iidx]
        clrs[1][2] = (clrs[1][iidx] + clrs[1][5])
        clrs[55][2] = (clrs[55][iidx] + clrs[55][5])
        clrs[99][2] = (clrs[99][iidx] + clrs[99][5])
        imgl = 0
    }
                                                    ...
                                                    VStack {
                                                        HStack {
                                                            ...
                                                            TextField("Playlist", text:$name)
                                                                ...
                                                                .padding(EdgeInsets(top:1.5, leading:12.0, bottom:1.5, trailing:24.0))
                                                                .overlay(RoundedRectangle(cornerRadius:9.0).inset(by:-3.5).stroke(colr(k:clrs[51][2]), lineWidth:3.0))
                                                                .offset(x:3.0, y:0.5)
                    ...
                    HStack {
                        VStack {
                            HStack {
                                                ...
                                                VStack {
                                                    VStack {
                                                        ...
                                                        HStack {
                                                            ...
                                                            Text(" ")
                                                            butv(kind:"multiply.circle", size:20.0, extr:[0.0, 0.0], iidx:91, clst:["b", "g", "b", "!"], meth:-1, pram:-1, actn:zill)
                                                            Text(" ")
                                                            Text(" ")
                                                            ColorPicker("Base", selection:$cbco).onChange(of:cbco) {
                                                                let temp = NSColor(cbco)
                                                                if (temp.alphaComponent > 0.0) {
                                                                    cbno = [1.0, temp.redComponent, temp.greenComponent, temp.blueComponent, 0.0]
                                                                    sldb = colr(k:"t")
                                                                    sldc = colr(k:"b")
                                                                    imgl = 0
                                                                }
                                                            ...
                                                            ColorPicker("High", selection:$chco).onChange(of:chco) {
                                                                ...
                                                                if (temp.alphaComponent > 0.0) {
                                                                    chno = [1.0, temp.redComponent, temp.greenComponent, temp.blueComponent, 0.0]
                                                                }
                                                            }
                                                            Text(" ")
                                                            ColorPicker("Text", selection:$ctco).onChange(of:ctco) {
                                                                ...
                                                                if (temp.alphaComponent > 0.0) {
                                                                    ctno = [1.0, temp.redComponent, temp.greenComponent, temp.blueComponent, 0.0]
                                                                }
                                                            }
                                                            Spacer()
                                                        }
                                ...
                                }.padding(.leading, 89.0).offset(x:-45.0+side[1], y:35.0)
                            }
                            Spacer()
                        }
                    }
    ...
    func shfu() -> Bool {
        if ((shuf[0] == false) && (shuf[1] == false)) {
            clrs[3][2] = (clrs[3][0] + clrs[3][5])
            return false
        } else {
            clrs[3][2] = (clrs[3][1] + clrs[3][5])
            return true
        }
    }
