...
extension String {
    func version() -> String { return "1.1.331" }
}
...
struct tabs: View {
    ...
    @State var rowr = 0.91
    @State var opac = 0.87
    @State var last = Date().timeIntervalSince1970
        ...
        ZStack {
            ZStack {
                ZStack {
                    ...
                    VStack(spacing:0) {
                        RoundedRectangle(cornerRadius:radi-magi).frame(maxHeight:.infinity).foregroundColor(objc.clrs.data_back.opacity(opac)).padding(EdgeInsets(top:-1*magi, leading:magi, bottom:-1*magi, trailing:magi))
                    }
                    VStack(spacing:0) {
                        Rectangle().frame(height:sizh+offz+offy).foregroundColor(objc.clrs.head_back.opacity(opac)).padding(EdgeInsets(top:-1*magi, leading:magi, bottom:-1*magi, trailing:magi))
                        Spacer()
                    }
                }
                VStack(spacing:0) {
                    GeometryReader { geome in
                        ScrollViewReader { proxy in
                            VStack(spacing:0) {
                                ScrollView([.horizontal], showsIndicators:false) {
                                    VStack(spacing:0) {
                                        Spacer()
                                        HStack {
                                            sepr(i:-1, h:sizh*sizr, w:geome.size.width, c:Color.clear).offset(y:-1*(offz/2.75))
                                            ForEach(0..<objc.cols.cols.count, id:\.self) { i in
                                                cold(i:i+1, j:-1, w:geome.size.width, r:refr, c:Color.clear).offset(y:-1.00)
                                                sepr(i:i+1, h:sizh*sizr, w:geome.size.width, c:objc.clrs.body_bord).offset(y:-1*(offz/2.75))
                                            }
                                            Color.clear.frame(width:aftr, height:1.0).contentShape(Rectangle())
                                            Spacer()
                                        }.frame(minWidth:geome.size.width)
                                            .offset(x:-sync)
                                        let offy = (-2.0 * objc.clrs.head_line_wide)
                                        line(i:1, j:-1, k:1, s:objc.clrs.head_line_wide, c:Color.clear, q:-1).offset(y:offy)
                                        Spacer()
                                    }
                                }.foregroundColor(Color.clear).background(Color.clear)
                                    .focusable().focusEffectDisabled().focused($isfo)
                                Spacer()
                            }.frame(height:sizh+offz).padding(.top, offz/1.9)
                            VStack(spacing:0) {
                                    ...
                                    VStack(spacing:0) {
                                        LazyVStack(spacing:0) {
                                            ...
                                            ForEach(0..<objc.rows.count, id:\.self) { j in
                                                ...
                                                HStack {
                                                    sepr(i:-1, h:sizh*sizr, w:geome.size.width, c:Color.clear)
                                                    ForEach(0..<objc.rows[j].count, id:\.self) { i in
                                                        rowd(i:i+1, j:j+1, w:geome.size.width, r:refr, c:Color.clear)
                                                        sepr(i:i+1, h:sizh*sizr, w:geome.size.width, c:Color.clear)
                                                    }
                                                    Color.clear.frame(width:aftr, height:1.0).contentShape(Rectangle())
                                                    Spacer()
                            ...
                            VStack(spacing:0) {  }.onChange(of:scro) {
                                if (scro != nil) {
                                    withAnimation {
                ...
                }.padding(EdgeInsets(top:0.0, leading:objc.clrs.body_wide, bottom:0.0, trailing:objc.clrs.body_wide))
            }.padding(EdgeInsets(top:objc.clrs.body_wide, leading:0.0, bottom:objc.clrs.body_wide, trailing:0.0))
            .overlay(RoundedRectangle(cornerRadius:objc.clrs.body_radi).inset(by:0.0).stroke(objc.clrs.body_bord.opacity(opac), lineWidth:objc.clrs.body_wide))
    ...
    func sepr(i:Int, h:CGFloat, w:CGFloat, c:Color) -> AnyView {
    ...
    func line(i:Int, j:Int, k:Int, s:CGFloat, c:Color, q:Double) -> some View {
        ...
        return VStack(spacing:0) {
            if ((k == 0) || (k == 2)) { RoundedRectangle(cornerRadius:radr).stroke(z, lineWidth:s).foregroundColor(Color.clear).frame(height:0.99).padding([.leading, .trailing], q * rowp) }
            if (k == 2) { Spacer() }
            if ((k == 1) || (k == 2)) { RoundedRectangle(cornerRadius:radr).stroke(z, lineWidth:s).foregroundColor(Color.clear).frame(height:0.99).padding([.leading, .trailing], q * rowp) }
        }
    }
...
class note: NotificationCenter, @unchecked Sendable {
    ...
    func loop() {
        ...
        while (0 == 0) {
            ...
            let chka = geth()
            let chkb = objc[keyPath:sees]
            if (chka == nil) {
                if (chkb != nil) {
                    objc[keyPath:sees] = nil
                }
            } else {
                if ((chkb == nil) || (chkb! != chka!.id)) {
                    objc[keyPath:sees] = chka!.id
                }
            }
            let numt = tabt.count
            let numb = tabp.count
                ...
                if ((secs - last) >= 3) {
                    ...
                    if (flag == 1) {
                        lock.withLock {
                            ...
                            for item in tabp {
                                if (selx != nil) {
                                    if (item.hash == selx!.hash) {
                                        objc[keyPath:sels] = selx!.id
                                    }
                                }
                                if (seex != nil) {
                                    if (item.hash == seex!.hash) {
                                        indx = iidx
                ...
                if (burl!.startAccessingSecurityScopedResource()) {
                    ...
                    if (temp != "") { //if let temp = try? exec(cmdl) {
                        ...
                        if (outp != "") {
                            ...
                            iidx = 0
                            var lnum = 0
                            while ((iidx < list.count) && (lnum < 5)) {
                                usleep(675000)
                                quel.withLock {
                                    while ((iidx < list.count) && (tabt[iidx].path != "")) {
                                        if (tabt[iidx].dobj > ldat) {
                                            ldat = tabt[iidx].dobj
                                            relo = 1
                                        }
                                        iidx += 1
                                ...
                                lnum += 1
                                print(Date(),"DEBUG","wait",iidx,list.count,lnum)
                            }
                            quel.withLock {
                                var tmpl = [] as [mdat]
                                for item in tabt {
    ...
    func meta(iidx:Int, path:String) {
        ...
        Task {
            let maps = [["title", "©nam"], ["artist", "TPE2", "©ART"], ["albumName", "©alb"], ["type", "©gen"], ["TYE", "TYER", "TDRC", "©day"]]
            var info = [[nils, mkda()], [nils, mkda()], [nils, mkda()], [nils, mkda()], [nils, mkda()]]
...
struct ContentView: View {
            ...
            VStack {
                HStack {
                    VStack {
                        let offs = 1.99
                        HStack {
                            butv(kind:"arrow.left.circle", size:37.0, extr:[epad*1.15, 0.0], iidx:0, clst:["b", "g", "b", "!"], meth:-1, pram:-1, actn:prev, actc:niln)
                            butv(kind:plyi(), size:45.0, extr:[epad*1.15, 0.0], iidx:idxs[5], clst:["b", "g", "b", "*"], meth:-1, pram:-1, actn:bply, actc:plyz)
                            butv(kind:"arrow.right.circle", size:37.0, extr:[epad*1.15, 0.0], iidx:2, clst:["b", "g", "b", "!"], meth:-1, pram:-1, actn:more, actc:niln)
                        }.offset(x:0.00, y:-1.99+offs).overlay(
                            butv(kind:"shuffle.circle", size:28.0, extr:[0.0, 0.0], iidx:3, clst:["b", "g", "b", "*"], meth:-1, pram:-1, actn:shfs, actc:shfz)
                                .offset(x:103.99, y:-2.19+offs)
                            )
                        HStack {  }.overlay(
                            HStack {
                                let spcr = 1.75
                                butv(kind:muti(), size:24.0, extr:[epad*spcr, 0.0], iidx:75, clst:["b", "g", "b", "~"], meth:-1, pram:-1, actn:minv, actc:mutz)
                                ...
                                butv(kind:"speaker.wave.2.circle", size:24.0, extr:[epad*spcr, 0.0], iidx:77, clst:["b", "g", "b", "!"], meth:-1, pram:-1, actn:maxv, actc:niln)
                    ...
                    VStack {
                        ...
                        HStack {
                            ...
                            TextField("Filter", text:$srch).disabled(fals).onAppear { DispatchQueue.main.async { fals = false } }
                                ...
                                .offset(x:3.99)
                            butv(kind:shwi(), size:30.0, extr:[0.0, 0.0], iidx:idxs[4], clst:["b", "g", "b", "~"], meth:-1, pram:-1, actn:shwa, actc:shwz).offset(x:12.0).overlay(
                                butv(kind:"multiply.circle", size:26.0, extr:[0.0, 0.0], iidx:5, clst:["b", "g", "b", "!"], meth:3, pram:3, actn:nill, actc:niln)
                                    .padding(.trailing, 1.0).padding(.top, 1.0).offset(x:13.00, y:41.00-offs))
                        }.offset(x:0.09, y:-1.99+offs)
                        HStack {  }.overlay(
                            HStack {
                                let spcr = 1.55
                                butv(kind:"star.circle", size:26.0, extr:[epad*spcr, 0.0], iidx:6, clst:["b", "g", "b", "!"], meth:-1, pram:-1, actn:star, actc:niln)
                                butv(kind:"viewfinder.circle", size:26.0, extr:[epad*spcr, 0.0], iidx:7, clst:["b", "g", "b", "!"], meth:-1, pram:-1, actn:fndr, actc:niln)
                                butv(kind:shoi(), size:26.0, extr:[epad*spcr, 0.0], iidx:8, clst:["b", "g", "b", "*"], meth:-1, pram:-1, actn:shot, actc:shoz)
                                butv(kind:shsi(), size:26.0, extr:[epad*spcr, 0.0], iidx:9, clst:["b", "g", "b", "~"], meth:-1, pram:-1, actn:shsa, actc:shsz)
    ...
    func shsi() -> String {
        if (shsz() == 0) { return "gearshape.circle" }
        return "minus.circle"
    }
    ...
    func shoi() -> String {
        if (shoz() == 0) { return "line.3.horizontal.circle" }
        return "minus.circle"
    }
                                                ...
                                                HStack {
                                                    VStack {
                                                        HStack {
                                                            ...
                                                            butv(kind:"plus.circle", size:24.0, extr:[0.0, 0.0], iidx:11, clst:["b", "g", "b", "!"], meth:-1, pram:-1, actn:padd, actc:niln)
                                                            Rectangle().foregroundColor(noco()).frame(width:5.9, height:1.0)
                                                        }
                                                        HStack {
                                                            butv(kind:"a.circle", size:20.0, extr:[0.0, 0.0], iidx:16, clst:["fa", "g", "fa", "!"], meth:0, pram:0, actn:nill, actc:niln)
                                                            butv(kind:"b.circle", size:20.0, extr:[0.0, 0.0], iidx:17, clst:["fb", "g", "fb", "!"], meth:0, pram:1, actn:nill, actc:niln)
                                                            butv(kind:"g.circle", size:20.0, extr:[0.0, 0.0], iidx:18, clst:["fg", "g", "fg", "!"], meth:0, pram:2, actn:nill, actc:niln)
                                                            butv(kind:"y.circle", size:20.0, extr:[0.0, 0.0], iidx:19, clst:["fo", "g", "fo", "!"], meth:0, pram:3, actn:nill, actc:niln)
                                                        ...
                                                        List {
                                                            ForEach(0..<plst.count, id:\.self) { i in
                                                                HStack {
                                                                    ...
                                                                    Spacer()
                                                                    butv(kind:"pencil.circle", size:20.0, extr:[0.0, 0.0], iidx:100+i, clst:["b", "g", "b", "!"], meth:1, pram:i, actn:nill, actc:niln)
                                                                    butv(kind:"multiply.circle", size:20.0, extr:[0.0, 0.0], iidx:200+i, clst:["b", "g", "b", "!"], meth:2, pram:i, actn:nill, actc:niln)
                                                        ...
                                                        HStack {
                                                            VStack {
                                                                ...
                                                                HStack {
                                                                    ...
                                                                    Text(" ")
                                                                    butv(kind:"multiply.circle", size:20.0, extr:[0.0, 0.0], iidx:91, clst:["b", "g", "b", "!"], meth:-1, pram:-1, actn:nilz, actc:niln)
                                                                    Spacer()
                                                                }
                                                                Spacer()
                                                        ...
                                                        HStack {
                                                            ...
                                                            VStack {
                                                                ...
                                                                HStack {
                                                                    ...
                                                                    Text(" ")
                                                                    butv(kind:"multiply.circle", size:20.0, extr:[0.0, 0.0], iidx:90, clst:["b", "g", "b", "!"], meth:-1, pram:-1, actn:zilw, actc:niln)
                                                                    Spacer()
                                                                }
                                                                Spacer()
                                                            }.frame(maxWidth:maxw).frame(maxHeight:maxh)
    ...
    func butv(kind:String, size:CGFloat, extr:[CGFloat], iidx:Int, clst:[String], meth:Int, pram:Int, actn:@escaping () -> Void, actc:@escaping () -> Int) -> some View {
